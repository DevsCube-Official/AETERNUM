...
tag @n[tag=aeternum.backstab.hurt,sort=nearest,limit=1] add aeternum.backstab.hurt_nearest

scoreboard players set .range aeternum.backstab.raycast 12
function aeternum:backstab/raycast
execute if score .range aeternum.backstab.raycast matches 0 as @n[tag=aeternum.backstab.hurt_nearest] run function aeternum:backstab/at_target
